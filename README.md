...
## Lesson summary → AI skill review

In an instructor lesson, use **Review skills from lesson summary**. Save a private summary, then **Find relevant skills**. This sends the typed summary and the fixed 664-item syllabus to OpenAI; it does not send pupil records, names, email addresses, routes or existing notes automatically. Avoid entering personal details in the summary. The OpenAI request has `store: false`; this is not a promise of zero provider retention. Summaries are saved in `lesson_skill_reviews`, visible only to instructors. Suggestions remain in the current browser session.

Suggestions include a reason and current rating. **Keep current** is the default. Choose new ratings and optionally tick **Pin this skill to this lesson**, or select whole core headings below. **Save selected changes** applies the selected ratings and pins in one database transaction. A stale rating rejects the whole transaction. You can pin without changing a rating. Pins contain the saved rating (or core heading average) after the changes; later progress edits do not change that snapshot. Existing pins are preserved; use **Update saved rating/score** to refresh them. Pupils can see saved pins and updated progress, but cannot see the private summary or AI reasoning.

Setup:
1. Apply `supabase/migrations/202609180003_ai_skill_review.sql` after the earlier migrations.
2. Deploy `supabase/functions/suggest-skills/index.ts` as **suggest-skills**. Disable **Verify JWT with legacy secret** for this function: it validates the user's JWT using Supabase Auth and verifies instructor membership itself, before any AI call. CLI equivalent: `supabase functions deploy suggest-skills --no-verify-jwt`.
3. In Supabase **Edge Functions → Secrets**, add **OPENAI_API_KEY** with your OpenAI platform API key. Keep it out of frontend code, GitHub and messages. Keep **ALLOWED_ORIGIN** as `https://oarandz.github.io` (no trailing slash or repository path).
4. The selected model is **gpt-4.1-mini-2025-04-14**, using the Responses API with strict structured output. API billing/credit and model permissions must be enabled on the OpenAI project; ChatGPT subscriptions do not configure this key.
5. Upload the updated website files, then test **Find relevant skills** on a fictional summary. The local sample workspace uses clearly labelled fixed examples and makes no OpenAI calls.

Limits: 6,000 summary characters; up to 80 suggestions per request; 30 seconds between requests and 100 requests per instructor per UTC day. These are request limits, not a billing budget. Failed upstream requests consume a slot. The edge function never writes ratings. Database functions validate skills, roles, lesson status and stale values separately when the instructor saves.
