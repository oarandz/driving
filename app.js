...
import {localDate,addDays,weekStart,minutes,lessonTotals,lessonMiles,routeMiles,previousAims,validateBooking,splitTrack,extractGpxPoints,youtubeVideoId,escapeHtml as h} from './core.js';
...
const state={demo:!config.supabaseUrl,role:'admin',view:'diary',week:weekStart(today),pupils:people,lessons:[sampleLesson('l1','p1',1,'09',2),sampleLesson('l2','p2',1,'12',1.5),sampleLesson('l3','p3',2,'10',2),sampleLesson('l4','p1',3,'14',2),sampleLesson('l5','p2',4,'09',2)],resources:[],links:[],videos:[],points:[],selected:null,busy:false};
...
async function reloadData(){if(state.demo)return;const [pupils,lessons,resources,links,videos]=await Promise.all(['pupils','lessons','resources','lesson_resources','lesson_videos'].map(allRows));Object.assign(state,{pupils,lessons,resources,links,videos});}
