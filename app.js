...
import {localDate,addDays,weekStart,minutes,lessonTotals,lessonMiles,routeMiles,previousAims,validateBooking,splitTrack,extractGpxPoints,youtubeVideoId,skillLevels,drivingSkillGroups,skillProgress,detailedSkillGroups,detailSkillProgress,escapeHtml as h} from './core.js';
...
async function reloadData(){if(state.demo)return;const [pupils,lessons,resources,links,videos,skills,activity,detailSkills,skillPins,skillReviews,detailPins]=await Promise.all(['pupils','lessons','resources','lesson_resources','lesson_videos','pupil_skill_ratings','pupil_activity','pupil_detail_ratings','lesson_skill_pins','lesson_skill_reviews','lesson_detail_skill_pins'].map(allRows));Object.assign(state,{pupils,lessons,resources,links,videos,skills,activity,detailSkills,skillPins,skillReviews,detailPins});}
...
function skillProgressMarkup(id){
 const {points,maximum,percent,introduced,independent,count}=detailSkillProgress(state.detailSkills||[],id);
 return `<div class="skill-progress"><div class="skill-progress-heading"><span>Skill progress</span><strong>${percent}%</strong></div><progress max="${maximum}" value="${points}" aria-label="Skill progress" aria-valuetext="${percent}%">${percent}%</progress><p class="help">${introduced} of ${count} introduced · ${independent} fully independent</p></div>`;
}
function coreSkillSummary(group,id){
 const p=detailSkillProgress(state.detailSkills||[],id,group.id);
 return `<span class="core-skill-title">${group.id}. ${h(group.name)}</span><span class="core-skill-score">${p.percent}% <small>${p.average.toFixed(2)} / 4</small></span><progress value="${p.points}" max="${p.maximum}" aria-label="${h(group.name)} progress">${p.percent}%</progress><span class="help">${p.independent} of ${p.count} fully independent · View detail</span>`;
}
function legacySkillMarkup(id){
 const ratings=(state.skills||[]).filter(r=>r.pupil_id===id);if(!ratings.length)return '';
 const names=drivingSkillGroups.flatMap(g=>g.skills);
 return `<details class="legacy-skills panel"><summary>Previous 27-skill ratings</summary><p class="help">Kept for reference. New detailed skills start at 0 and are graded separately.</p><div class="list">${ratings.sort((a,b)=>a.skill_id-b.skill_id).map(r=>`<div class="skill-row skill-level-${r.rating}"><span>${r.skill_id}. ${h(names[r.skill_id-1])}</span><span>${r.rating} — ${h(skillLevels[r.rating])}</span></div>`).join('')}</div></details>`;
}
function renderSkills(){
 const id=state.role==='admin'?state.progressPupil:(state.pupilId||'p1'),p=pupil(id);if(!p)return;
 const admin=state.role==='admin';
 $('#content').innerHTML=`<div class="heading"><div><h1>Driving skills</h1>${admin?`<p>${h(p.name)}</p>`:''}</div><div class="profile-actions"><button id="refresh-skills">Refresh ratings</button><button id="back-from-skills">${admin?'Back to pupil':'Back to my progress'}</button></div></div><div id="skills-progress" class="skills-progress-panel">${skillProgressMarkup(id)}</div><p class="help">36 core skills · 664 detailed skills · Select a heading to view its detail.</p><div class="skill-scale">${skillLevels.map((label,i)=>`<span><b class="skill-level-${i}">${i}</b> ${h(label)}</span>`).join('')}</div><label class="skill-search">Find a skill<input id="skill-search" type="search" placeholder="For example: roundabouts or mirrors"></label><div class="core-skill-groups">${detailedSkillGroups.map(group=>`<details class="panel core-skill-group" data-category="${group.id}"><summary id="core-summary-${group.id}">${coreSkillSummary(group,id)}</summary><div class="core-skill-detail">${group.skills.map((name,index)=>{const item=index+1,key=group.id+'-'+item,r=(state.detailSkills||[]).find(x=>x.pupil_id===id&&x.category_id===group.id&&x.item_id===item),rating=r?.rating??0;return `<div class="skill-row skill-level-${rating}" data-item-name="${h(name.toLowerCase())}"><div><label id="skill-label-${key}" ${admin?`for="skill-${key}"`:''}>${group.id}.${item} ${h(name)}</label><p class="help" id="skill-status-${key}" role="status">${r?'Updated '+h(activityDate(r.updated_at)):''}</p></div>${admin?`<select id="skill-${key}" data-detail-skill="${key}" aria-labelledby="skill-label-${key}">${skillLevels.map((label,i)=>`<option value="${i}" ${i===rating?'selected':''}>${i} — ${h(label)}</option>`).join('')}</select>`:`<span class="skill-rating">${rating} — ${h(skillLevels[rating])}</span>`}</div>`;}).join('')}</div></details>`).join('')}</div><p id="no-skill-results" class="empty" hidden>No matching skills.</p>${legacySkillMarkup(id)}`;
 $('#back-from-skills').onclick=()=>{state.view='progress';render();};
 $('#refresh-skills').onclick=()=>safely(async()=>{await reloadData();renderSkills();});
 $('#skill-search').oninput=e=>{
  const term=e.target.value.trim().toLowerCase();let matches=0;
  document.querySelectorAll('[data-category]').forEach(section=>{
   const group=detailedSkillGroups.find(g=>g.id===Number(section.dataset.category)),categoryMatches=group.name.toLowerCase().includes(term)||String(group.id)===term;let shown=0;
   section.querySelectorAll('[data-item-name]').forEach(row=>{row.hidden=Boolean(term&&!categoryMatches&&!row.dataset.itemName.includes(term));if(!row.hidden)shown++;});
   section.hidden=!shown;section.open=Boolean(term&&shown);if(shown)matches++;
  });$('#no-skill-results').hidden=matches>0;
 };
 if(admin)document.querySelectorAll('[data-detail-skill]').forEach(select=>select.onchange=async()=>{
  const [category,item]=select.dataset.detailSkill.split('-').map(Number),key=category+'-'+item,old=(state.detailSkills||[]).find(x=>x.pupil_id===id&&x.category_id===category&&x.item_id===item),expected=old?.rating??0,rating=Number(select.value),status=$('#skill-status-'+key),row=select.closest('.skill-row'),progress=$('#skills-progress'),summary=$('#core-summary-'+category);
  if(state.role!=='admin'){select.value=String(expected);return;}
  select.disabled=true;status.textContent='Saving…';
  try{
   const result=state.demo?{id:old?.id||crypto.randomUUID(),pupil_id:id,category_id:category,item_id:item,rating,updated_at:new Date().toISOString()}:await rpc('set_pupil_detail_rating',{p_pupil_id:id,p_category_id:category,p_item_id:item,p_rating:rating,p_expected:expected});
   const saved=Array.isArray(result)?result[0]:result;
   state.detailSkills=state.detailSkills.filter(x=>!(x.pupil_id===id&&x.category_id===category&&x.item_id===item));state.detailSkills.push(saved);status.textContent='Saved · '+activityDate(saved.updated_at);row.className='skill-row skill-level-'+saved.rating;if(progress?.isConnected)progress.innerHTML=skillProgressMarkup(id);if(summary?.isConnected)summary.innerHTML=coreSkillSummary(detailedSkillGroups.find(g=>g.id===category),id);
  }catch(e){select.value=String(expected);status.textContent='Not saved: '+(e.message||'Please try again.');}
  finally{select.disabled=false;}
 });
}
function lessonSkillControls(){return `<details class="lesson-resource-editor"><summary>＋ Pin a core skill</summary><form id="pin-skill-form" class="stack"><label>Core skill<select name="category" required><option value="">Choose a skill</option>${detailedSkillGroups.map(g=>`<option value="${g.id}">${g.id}. ${h(g.name)}</option>`).join('')}</select></label><p class="help">Saves the current average of its detailed ratings in this lesson. Later rating changes do not alter this saved score.</p><button class="primary" type="submit">Pin skill to lesson</button><p id="pin-skill-status" class="help" role="status"></p></form></details>`;}
async function saveLessonSkillPin(lessonId,category,refresh=false){
 const l=visibleLessons().find(x=>x.id===lessonId);if(!l||state.role!=='admin')throw Error('Instructor access required');if(l.status==='cancelled')throw Error('Lesson unavailable');
 if(state.demo){
  const old=state.skillPins.find(x=>x.lesson_id===lessonId&&x.category_id===category);if(old?.pinned&&!refresh)return;
  const stats=detailSkillProgress(state.detailSkills,l.pupil_id,category),saved={id:old?.id||crypto.randomUUID(),lesson_id:lessonId,category_id:category,title:detailedSkillGroups.find(g=>g.id===category).name,points:stats.points,item_count:stats.count,captured_at:new Date().toISOString(),pinned:true};
  state.skillPins=state.skillPins.filter(x=>x.id!==saved.id);state.skillPins.push(saved);
 }else{const result=await rpc('pin_lesson_skill',{p_lesson_id:lessonId,p_category_id:category,p_refresh:refresh}),saved=Array.isArray(result)?result[0]:result;state.skillPins=state.skillPins.filter(x=>x.id!==saved.id);state.skillPins.push(saved);}
}
function showLessonSkills(id){
 const root=$('#lesson-skills');if(!root)return;const admin=state.role==='admin',pins=(state.skillPins||[]).filter(x=>x.lesson_id===id&&x.pinned).sort((a,b)=>a.category_id-b.category_id);
 root.innerHTML=pins.map(pin=>{const average=pin.points/pin.item_count,level=average===4?4:Math.floor(average);return `<article class="lesson-skill-pin skill-level-${level}"><div><strong>${pin.category_id}. ${h(pin.title)}</strong><p>Saved score: ${average.toFixed(2)} / 4</p><small>${pin.points} of ${pin.item_count*4} points · Saved ${h(activityDate(pin.captured_at))}</small></div>${admin?`<div class="profile-actions"><button data-refresh-skill-pin="${pin.category_id}">Update saved score</button><button data-unpin-skill="${pin.id}">Unpin skill</button></div>`:''}</article>`;}).join('');
 root.innerHTML+=lessonDetailPinsMarkup(id);if(!root.innerHTML)root.innerHTML='<p class="help">No skills pinned to this lesson.</p>';
 bindLessonDetailPins(id,root);
 root.querySelectorAll('[data-refresh-skill-pin]').forEach(b=>b.onclick=()=>safely(async()=>{await saveLessonSkillPin(id,Number(b.dataset.refreshSkillPin),true);if(root.isConnected)showLessonSkills(id);toast('Saved lesson score updated.');}));
 root.querySelectorAll('[data-unpin-skill]').forEach(b=>b.onclick=()=>safely(async()=>{if(state.role!=='admin')return;if(!state.demo)await rpc('unpin_lesson_skill',{p_id:b.dataset.unpinSkill});state.skillPins.find(x=>x.id===b.dataset.unpinSkill).pinned=false;if(root.isConnected)showLessonSkills(id);}));
 const form=$('#pin-skill-form');if(form)form.onsubmit=e=>{e.preventDefault();safely(async()=>{const status=$('#pin-skill-status');try{await saveLessonSkillPin(id,Number(form.elements.category.value));if(root.isConnected){showLessonSkills(id);form.reset();status.textContent='Skill pinned. Your pupil can see the saved score.';}}catch(e){status.textContent=e.message;}});};
}
async function saveLessonDetailPin(id,category,item,refresh=false){
 const l=visibleLessons().find(x=>x.id===id),group=detailedSkillGroups.find(g=>g.id===category);if(!l||state.role!=='admin'||state.pupilPreview||l.status==='cancelled'||!group?.skills[item-1])throw Error('Skill unavailable');
 let saved;
 if(state.demo){const old=state.detailPins.find(p=>p.lesson_id===id&&p.category_id===category&&p.item_id===item);if(old?.pinned&&!refresh)return;saved={id:old?.id||crypto.randomUUID(),lesson_id:id,category_id:category,item_id:item,title:group.skills[item-1],rating:state.detailSkills.find(r=>r.pupil_id===l.pupil_id&&r.category_id===category&&r.item_id===item)?.rating??0,captured_at:new Date().toISOString(),pinned:true};}
 else{const result=await rpc('pin_lesson_detail_skill',{p_lesson_id:id,p_category:category,p_item:item,p_title:group.skills[item-1],p_refresh:refresh});saved=Array.isArray(result)?result[0]:result;}
 state.detailPins=state.detailPins.filter(x=>x.id!==saved.id);state.detailPins.push(saved);
}
function lessonDetailPinsMarkup(id){
 return (state.detailPins||[]).filter(p=>p.lesson_id===id&&p.pinned).sort((a,b)=>a.category_id-b.category_id||a.item_id-b.item_id).map(pin=>`<article class="lesson-skill-pin skill-level-${pin.rating}"><div><small>${h(detailedSkillGroups.find(g=>g.id===pin.category_id)?.name||'')}</small><strong>${pin.category_id}.${pin.item_id} ${h(pin.title)}</strong><p>Saved rating: ${pin.rating} — ${h(skillLevels[pin.rating])}</p><small>Saved ${h(activityDate(pin.captured_at))}</small></div>${state.role==='admin'?`<div class="profile-actions"><button data-refresh-detail-pin="${pin.id}">Update saved rating</button><button data-unpin-detail="${pin.id}">Unpin skill</button></div>`:''}</article>`).join('');
}
function bindLessonDetailPins(id,root){
 root.querySelectorAll('[data-refresh-detail-pin]').forEach(b=>b.onclick=()=>safely(async()=>{const pin=state.detailPins.find(x=>x.id===b.dataset.refreshDetailPin);await saveLessonDetailPin(id,pin.category_id,pin.item_id,true);if(root.isConnected){showLessonSkills(id);showSkillReviewResults(id);}toast('Saved lesson rating updated.');}));
 root.querySelectorAll('[data-unpin-detail]').forEach(b=>b.onclick=()=>safely(async()=>{if(state.role!=='admin'||state.pupilPreview)return;if(!state.demo)await rpc('unpin_lesson_detail_skill',{p_id:b.dataset.unpinDetail});state.detailPins.find(p=>p.id===b.dataset.unpinDetail).pinned=false;if(root.isConnected){showLessonSkills(id);showSkillReviewResults(id);}}));
}

const skillReviewDrafts=new Map();
function lessonSkillReviewMarkup(l){
 const saved=(state.skillReviews||[]).find(r=>r.lesson_id===l.id),draft=skillReviewDrafts.get(l.id);
 return `<section class="ai-skill-review" aria-labelledby="ai-review-heading"><h3 id="ai-review-heading">Review skills from lesson summary</h3><p class="help">Instructor only. Describe key events, improvements and any help needed.</p><form id="skill-summary-form" class="stack"><label>Lesson summary<textarea id="skill-review-summary" maxlength="6000" rows="5" placeholder="For example: Needed reminders to check mirrors before slowing. Judged gaps at roundabouts independently.">${h(draft?.summary??saved?.summary??'')}</textarea></label><p class="help">Find relevant skills sends this summary and the skill list to OpenAI. Leave out names and personal details. You choose the ratings.</p><div class="profile-actions"><button type="submit">Save summary</button><button type="button" class="primary" id="suggest-skills">${state.demo?'Show sample suggestions':'Find relevant skills'}</button></div><p id="skill-review-status" class="help" role="status"></p></form><div id="skill-review-results"></div></section>`;
}
function normaliseSkillSuggestions(suggestions){
 if(!Array.isArray(suggestions)||suggestions.length>80)throw Error('Invalid skill suggestions. Try again.');
 const seen=new Set();
 return suggestions.map(s=>{
  if(typeof s?.skill_id!=='string'||!/^\d+\.\d+$/.test(s.skill_id))throw Error('Invalid skill suggestion. Try again.');
  const [category,item]=s.skill_id.split('.').map(Number),group=detailedSkillGroups.find(g=>g.id===category);
  if(!group?.skills[item-1]||s.skill_id!==`${category}.${item}`||seen.has(s.skill_id)||typeof s.reason!=='string'||!s.reason.trim()||s.reason.length>500)throw Error('Invalid skill suggestion. Try again.');
  seen.add(s.skill_id);return {category_id:category,item_id:item,reason:s.reason};
 });
}
async function requestSkillSuggestions(id,summary){
 if(state.role!=='admin'||state.pupilPreview)throw Error('Instructor access required');
 if(state.demo)return [{skill_id:'9.6',reason:'Example: reminders were needed to check mirrors before slowing.'},{skill_id:'19.10',reason:'Example: roundabout gaps were judged independently.'},{skill_id:'19.7',reason:'Example: approach speed needed prompting.'}];
 const {data,error}=await db.functions.invoke('suggest-skills',{body:{lesson_id:id,summary}});
 if(error){let message='Could not get suggestions. Your summary is saved. Try again.';try{message=(await error.context.json()).error||message;}catch{}throw Error(message);}
 if(data?.error)throw Error(data.error);return data?.suggestions;
}
async function applySkillReview(id,changes,pinCategories=[],pinItems=[]){
 const l=visibleLessons().find(x=>x.id===id);if(state.role!=='admin'||state.pupilPreview||!l||l.status==='cancelled')throw Error('Instructor access required');
 if(!Array.isArray(changes)||changes.length>80||!Array.isArray(pinCategories)||pinCategories.length>36||!Array.isArray(pinItems)||pinItems.length>80||!changes.length&&!pinCategories.length&&!pinItems.length)throw Error('Choose a rating change or a skill to pin.');
 if(new Set(pinCategories).size!==pinCategories.length||pinCategories.some(c=>!Number.isInteger(c)||!detailedSkillGroups.some(g=>g.id===c)))throw Error('Invalid skills to pin');
 const itemKeys=new Set();for(const item of pinItems){const key=item.category_id+'.'+item.item_id,group=detailedSkillGroups.find(g=>g.id===item.category_id);if(!Number.isInteger(item.item_id)||!group?.skills[item.item_id-1]||itemKeys.has(key))throw Error('Invalid skill to pin');itemKeys.add(key);item.title=group.skills[item.item_id-1];}
 const seen=new Set();
 for(const c of changes){
  const group=detailedSkillGroups.find(g=>g.id===c.category_id),key=c.category_id+'.'+c.item_id;
  if(!Number.isInteger(c.item_id)||!group?.skills[c.item_id-1]||![c.rating,c.expected].every(n=>Number.isInteger(n)&&n>=0&&n<=4)||seen.has(key))throw Error('Invalid rating changes');seen.add(key);
  const current=state.detailSkills.find(r=>r.pupil_id===l.pupil_id&&r.category_id===c.category_id&&r.item_id===c.item_id)?.rating??0;
  if(current!==c.expected)throw Error('A rating has changed. Refresh the current ratings and review your choices again.');
 }
 const result=state.demo?{ratings:changes.map(c=>({id:crypto.randomUUID(),pupil_id:l.pupil_id,category_id:c.category_id,item_id:c.item_id,rating:c.rating,updated_at:new Date().toISOString()})),pins:[],detail_pins:[]}:await rpc('apply_lesson_skill_ratings',{p_lesson_id:id,p_changes:changes,p_pin_categories:pinCategories,p_pin_items:pinItems});
 for(const r of result.ratings){state.detailSkills=state.detailSkills.filter(x=>!(x.pupil_id===r.pupil_id&&x.category_id===r.category_id&&x.item_id===r.item_id));state.detailSkills.push(r);}
 if(state.demo){for(const category of pinCategories)await saveLessonSkillPin(id,category);for(const item of pinItems)await saveLessonDetailPin(id,item.category_id,item.item_id);}else{for(const pin of result.pins){state.skillPins=state.skillPins.filter(x=>x.id!==pin.id);state.skillPins.push(pin);}for(const pin of result.detail_pins){state.detailPins=state.detailPins.filter(x=>x.id!==pin.id);state.detailPins.push(pin);}}
 return {ratingCount:result.ratings.length,pinCount:pinCategories.length+pinItems.length};
}
function showSkillReviewResults(id){
 const root=$('#skill-review-results'),draft=skillReviewDrafts.get(id),l=visibleLessons().find(x=>x.id===id);
 if(!root||!l||state.role!=='admin'||state.pupilPreview)return;
 if(!draft?.suggestions){root.innerHTML='';return;}
 if(!draft.suggestions.length){root.innerHTML='<p class="notice">No relevant skills found. Add specific driving events to the summary and try again.</p>';return;}
 const suggestions=draft.suggestions,categories=[...new Set(suggestions.map(s=>s.category_id))].sort((a,b)=>a-b);
 root.innerHTML=`<form id="skill-rating-review" class="stack"><h4>${suggestions.length} skills to review</h4>${state.demo?'<p class="notice">Sample suggestions for preview only. No AI request was made.</p>':''}<p class="help">Check each match. Keep current leaves that rating unchanged. Saving updates the pupil’s current progress; existing scores pinned to lessons stay as saved.</p><div class="ai-suggestions">${suggestions.map(s=>{const group=detailedSkillGroups.find(g=>g.id===s.category_id),key=s.category_id+'.'+s.item_id,rating=state.detailSkills.find(r=>r.pupil_id===l.pupil_id&&r.category_id===s.category_id&&r.item_id===s.item_id)?.rating??0;s.expected=rating;const alreadyPinned=state.detailPins.some(p=>p.lesson_id===id&&p.category_id===s.category_id&&p.item_id===s.item_id&&p.pinned);return `<article class="ai-suggestion skill-level-${rating}"><small>${s.category_id}. ${h(group.name)}</small><strong id="ai-label-${key.replace('.','-')}">${key} ${h(group.skills[s.item_id-1])}</strong><p>${h(s.reason)}</p><p class="help">Current: ${rating} — ${h(skillLevels[rating])}</p><label>New rating<select data-ai-skill="${key}" aria-labelledby="ai-label-${key.replace('.','-')}"><option value="">Keep current (${rating})</option>${skillLevels.map((label,i)=>`<option value="${i}" ${i===rating?'disabled':''}>${i} — ${h(label)}</option>`).join('')}</select></label><label class="ai-detail-pin"><input type="checkbox" data-ai-detail-pin="${key}" ${alreadyPinned?'disabled':''}>${alreadyPinned?'Already pinned to this lesson':'Pin this skill to this lesson'}</label></article>`;}).join('')}</div><fieldset class="ai-pin-options"><legend>Pin core skills to this lesson (optional)</legend><p class="help">Selected skills are pinned with their scores after the rating changes above.</p>${categories.map(category=>{const group=detailedSkillGroups.find(g=>g.id===category),pinned=state.skillPins.some(p=>p.lesson_id===id&&p.category_id===category&&p.pinned);return `<label><input type="checkbox" data-ai-pin="${category}" ${pinned?'disabled':''}>${category}. ${h(group.name)}${pinned?' <span class="help">Already pinned · use Update saved score below to refresh it.</span>':''}</label>`;}).join('')}</fieldset><div class="profile-actions"><button type="submit" class="primary" id="apply-skill-changes" disabled>Save selected changes</button><button type="button" id="refresh-review-ratings">Refresh current ratings</button></div><p id="skill-rating-status" class="help" role="status">No changes selected.</p></form>`;
 const form=$('#skill-rating-review'),status=$('#skill-rating-status'),button=$('#apply-skill-changes');
 const changes=()=>[...form.querySelectorAll('[data-ai-skill]')].filter(s=>s.value!=='').map(select=>{const [category_id,item_id]=select.dataset.aiSkill.split('.').map(Number);return {category_id,item_id,rating:Number(select.value),expected:suggestions.find(s=>s.category_id===category_id&&s.item_id===item_id).expected};});
 const pins=()=>[...form.querySelectorAll('[data-ai-pin]:checked')].map(input=>Number(input.dataset.aiPin));
 const itemPins=()=>[...form.querySelectorAll('[data-ai-detail-pin]:checked')].map(input=>{const [category_id,item_id]=input.dataset.aiDetailPin.split('.').map(Number);return {category_id,item_id};});
 form.onchange=()=>{const count=changes().length,pinCount=pins().length+itemPins().length;button.disabled=!(count+pinCount);button.textContent='Save selected changes';status.textContent=count+pinCount?`${count} rating change${count===1?'':'s'} · ${pinCount} skill${pinCount===1?'':'s'} to pin. Nothing saved yet.`:'No changes selected.';};
 form.onsubmit=e=>{e.preventDefault();safely(async()=>{
  if(draft.reviewedSummary!==draft.summary)throw Error('The summary has changed. Find relevant skills again.');
  const selected=changes(),selectedPins=pins(),selectedItems=itemPins();form.querySelectorAll('select,input,button').forEach(x=>x.disabled=true);
  try{const result=await applySkillReview(id,selected,selectedPins,selectedItems);if(root.isConnected&&state.selected===id){showSkillReviewResults(id);showLessonSkills(id);$('#skill-rating-status').textContent=`Saved: ${result.ratingCount} rating change${result.ratingCount===1?'':'s'} and ${result.pinCount} pinned skill${result.pinCount===1?'':'s'}.`;}}
  catch(error){if(status.isConnected)status.textContent=error.message;}
  finally{if(form.isConnected){form.querySelectorAll('select,input,button').forEach(x=>x.disabled=false);form.querySelectorAll('[data-ai-pin]').forEach(x=>{x.disabled=state.skillPins.some(p=>p.lesson_id===id&&p.category_id===Number(x.dataset.aiPin)&&p.pinned);});form.querySelectorAll('[data-ai-detail-pin]').forEach(x=>{const [category,item]=x.dataset.aiDetailPin.split('.').map(Number);x.disabled=state.detailPins.some(p=>p.lesson_id===id&&p.category_id===category&&p.item_id===item&&p.pinned);});button.disabled=!(changes().length+pins().length+itemPins().length);}}
 });};
 $('#refresh-review-ratings').onclick=()=>safely(async()=>{await reloadData();if(root.isConnected&&state.selected===id){showSkillReviewResults(id);$('#skill-rating-status').textContent='Current ratings refreshed. Choose any changes again.';}});
}
function bindLessonSkillReview(id){
 const form=$('#skill-summary-form');if(!form||state.role!=='admin')return;
 const input=$('#skill-review-summary'),status=$('#skill-review-status'),button=$('#suggest-skills');
 const saved=(state.skillReviews||[]).find(r=>r.lesson_id===id);
 let draft=skillReviewDrafts.get(id);if(!draft){draft={summary:saved?.summary||'',expected:saved?.summary||''};skillReviewDrafts.set(id,draft);}
 input.oninput=()=>{draft.summary=input.value;draft.suggestions=null;$('#skill-review-results').innerHTML='';status.textContent='Summary not saved.';};
 const save=async()=>{
  if(state.role!=='admin'||state.pupilPreview)throw Error('Instructor access required');
  const summary=input.value;if(summary.length>6000)throw Error('Summary must be at most 6000 characters');
  const result=state.demo?{id:saved?.id||id,lesson_id:id,summary}:await rpc('save_lesson_skill_summary',{p_lesson_id:id,p_summary:summary,p_expected:draft.expected});
  const row=Array.isArray(result)?result[0]:result;state.skillReviews=(state.skillReviews||[]).filter(x=>x.lesson_id!==id);state.skillReviews.push(row);draft.summary=summary;draft.expected=summary;
  if(status.isConnected)status.textContent='Summary saved. Only instructors can see it.';
 };
 form.onsubmit=e=>{e.preventDefault();safely(save);};
 button.onclick=()=>safely(async()=>{
  if(input.value.trim().length<10)throw Error('Enter at least 10 characters describing the lesson.');
  button.disabled=true;input.readOnly=true;
  try{await save();status.textContent=state.demo?'Loading sample suggestions…':'Finding relevant skills…';draft.suggestions=null;$('#skill-review-results').innerHTML='';const summary=input.value;
   const suggestions=normaliseSkillSuggestions(await requestSkillSuggestions(id,summary));await reloadData();draft.suggestions=suggestions;draft.reviewedSummary=summary;
   if(form.isConnected&&state.selected===id&&state.role==='admin'){status.textContent='Summary saved. Review the skills below.';showSkillReviewResults(id);}
  }catch(error){if(status.isConnected)status.textContent=error.message;}
  finally{button.disabled=false;input.readOnly=false;}
 });
 showSkillReviewResults(id);
}
...
function openLesson(id){
 ...
 showLessonResources(id);bindLessonResourceControls(id);showLessonSkills(id);if(admin&&l.status!=='cancelled')bindLessonSkillReview(id);
}
