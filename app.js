...
import {localDate,addDays,weekStart,minutes,lessonTotals,lessonMiles,routeMiles,previousAims,validateBooking,splitTrack,extractGpxPoints,youtubeVideoId,skillLevels,drivingSkillGroups,skillProgress,escapeHtml as h} from './core.js';
...
function skillProgressMarkup(id){
 const {points,maximum,percent}=skillProgress(state.skills||[],id);
 return `<div class="skill-progress"><div class="skill-progress-heading"><span>Skill progress</span><strong>${percent}%</strong></div><progress max="${maximum}" value="${points}" aria-label="Skill progress" aria-valuetext="${percent}%">${percent}%</progress><p class="help">${points} of ${maximum} points</p></div>`;
}
function renderSkills(){
 ...
 $('#content').innerHTML=`<div class="heading"><div><h1>Driving skills</h1>${admin?`<p>${h(p.name)}</p>`:''}</div><button id="back-from-skills">${admin?'Back to pupil':'Back to my progress'}</button></div><div id="skills-progress" class="skills-progress-panel">${skillProgressMarkup(id)}</div><p class="help">27 DVSA skills · Instructor rating scale 0–4</p><div class="skill-scale">${skillLevels.map((label,i)=>`<span><b class="skill-level-${i}">${i}</b> ${h(label)}</span>`).join('')}</div><div class="skill-groups">${drivingSkillGroups.map(group=>`<section class="panel skill-group"><h2>${h(group.name)}</h2>${group.skills.map(name=>{const n=++skillId,r=(state.skills||[]).find(x=>x.pupil_id===id&&x.skill_id===n),rating=r?.rating??0;return `<div class="skill-row skill-level-${rating}"><div><label id="skill-label-${n}" for="skill-${n}">${n}. ${h(name)}</label><p class="help" id="skill-status-${n}" role="status">${r?'Updated '+h(activityDate(r.updated_at)):''}</p></div>${admin?`<select id="skill-${n}" data-skill="${n}" aria-labelledby="skill-label-${n}">${skillLevels.map((label,i)=>`<option value="${i}" ${i===rating?'selected':''}>${i} — ${h(label)}</option>`).join('')}</select>`:`<span class="skill-rating rating-${rating}">${rating} — ${h(skillLevels[rating])}</span>`}</div>`;}).join('')}</section>`).join('')}</div><p class="help skill-source">Skill names: <a href="https://readytopass.campaign.gov.uk/driving-skills/" target="_blank" rel="noopener noreferrer">DVSA</a>, Crown copyright, <a href="https://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/" target="_blank" rel="noopener noreferrer">Open Government Licence v3.0</a>. Ratings use your instructor’s scale.</p>`;
 ...
 if(admin)document.querySelectorAll('[data-skill]').forEach(select=>select.onchange=async()=>{
  const n=Number(select.dataset.skill),old=(state.skills||[]).find(x=>x.pupil_id===id&&x.skill_id===n),expected=old?.rating??0,rating=Number(select.value),status=$('#skill-status-'+n),row=select.closest('.skill-row'),progress=$('#skills-progress');
  ...
  try{
   ...
   state.skills=state.skills.filter(x=>!(x.pupil_id===id&&x.skill_id===n));state.skills.push(saved);status.textContent='Saved · '+activityDate(saved.updated_at);row.className='skill-row skill-level-'+saved.rating;if(progress?.isConnected)progress.innerHTML=skillProgressMarkup(id);
