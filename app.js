...
import {localDate,addDays,weekStart,minutes,lessonTotals,lessonMiles,routeMiles,previousAims,validateBooking,splitTrack,extractGpxPoints,escapeHtml as h} from './core.js';
...
const state={demo:!config.supabaseUrl,role:'admin',view:'diary',week:weekStart(today),pupils:people,lessons:[sampleLesson('l1','p1',1,'09',2),sampleLesson('l2','p2',1,'12',1.5),sampleLesson('l3','p3',2,'10',2),sampleLesson('l4','p1',3,'14',2),sampleLesson('l5','p2',4,'09',2)],resources:[],links:[],points:[],selected:null,busy:false};
...
async function openWorkspace(){
 try{
  ...
  await reloadData();if(state.role==='pupil'){state.pupilId=state.pupils[0]?.id;if(!state.pupilId){authScreen('Your account is not linked to a pupil yet. Ask your instructor for an access code.');return;}}
...
async function renderProgress(id){
 ...
 try{const points=(await Promise.all(lessons.filter(l=>l.status==='completed').map(l=>lessonPoints(l.id)))).flat();if(!$('#progress-map'))return;drawMap('progress-map',points,true);$('#fog').onchange=e=>drawMap('progress-map',points,e.target.checked);}catch(e){toast(e.message,true);}
}
...
function showLessonResources(id){const links=state.links.filter(x=>x.lesson_id===id);$('#lesson-resources').innerHTML=links.map(link=>{const r=state.resources.find(r=>r.id===link.resource_id);return r?`<button data-resource="${r.id}" class="list-button">${h(r.title)} <span>Open →</span></button>`:'';}).join('')||'<p class="help">No resources attached yet.</p>';$('#lesson-resources').querySelectorAll('button').forEach(b=>b.onclick=()=>safely(()=>openResource(b.dataset.resource,id)));}
