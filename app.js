...
function render(){progressMapRequest++;if(map){map.remove();map=null;}shell();if(state.view==='diary')renderDiary();else if(state.view==='pupils')renderPupils();else if(state.view==='resources')renderResources();else renderProgress(state.role==='admin'?state.progressPupil:(state.pupilId||'p1'));}
...
function openLesson(id){
 const l=visibleLessons().find(l=>l.id===id);if(!l)return;state.selected=id;const admin=state.role==='admin';
